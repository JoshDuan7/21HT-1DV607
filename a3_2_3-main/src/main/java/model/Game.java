...
public class Game {
  ...
  private GameRuleSetInterface rules;
  ...
  /**
   * Constructor that create a new game instance with a GameRuleSetInterface parameter.
   *
   * @param rules The rules which are applied in current game.
   */
  public Game(GameRuleSetInterface rules) {
    this.rules = rules;
  ...
  public void newCardObserver(InCardObserver observer) {
    dealer.initObserver(observer);
    player.initObserver(observer);
  }

  public GameRuleSetInterface getRules() {
    return rules;
  }
}
